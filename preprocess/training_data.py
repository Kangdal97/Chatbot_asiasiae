...
from kogpt2_transformers import get_kogpt2_tokenizer    ### GPT 모델 임포트.

def tweet_dialog_dataset():    ### 트위터 기반 대화 시나리오
  root_path = "../data"
...
if __name__ == "__main__":     ### 원본 데이터 chatbot_wellness_data.txt
  root_path = "../data"
  file_path = root_path + "/chatbot_wellness_data.txt"   #  전처리 전
  o_path = root_path + "/chatbot_wellness_data_for_autoregressive.txt"  # 전처리 후 GPT 모델을 위한 자료.

  i_file = open(file_path, 'r', encoding="utf-8")
